...
export default class RNCreditCard extends Component {
  ...
  onMomentumScrollEnd(e, state, context) {
    var indexMap = ['number', 'name', 'expiry', 'cvc', 'type'];
    this.setState(
      {
        index: state.index,
              ...
              <View style={{flexDirection: 'row'}}>
                {cardTypes.map(cardType => {
                  return (
                    <TouchableOpacity
                      key={cardType.type}
                      onPress={() => this.setState({type: cardType.type})}
                    >
                      <View>
                        <Image
                          source={{uri: cardType.image}}
                          style={{width: 57, height: 35, marginHorizontal: 5}}
                        />
                      </View>
                    </TouchableOpacity>
                  );
...
Expo.registerRootComponent(RNCreditCard);
